...
export async function getObjectAclPolicy(objectFile: File): Promise<ObjectAclPolicy | null> {
  try {
    ...
    if (metadata?.metadata?.owner && metadata?.metadata?.visibility) {
      return {
        owner: metadata.metadata.owner as string,
        visibility: metadata.metadata.visibility as "public" | "private",
      };
    }
  } catch {
    // Ignore errors
...
export async function canAccessObject({
  ...
  if (!aclPolicy) {
    return true; // Default to accessible if no policy
  }
